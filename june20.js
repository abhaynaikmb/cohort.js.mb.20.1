...
let emp=[{"id":1,"first_name":"Gregg","last_name":"Lacey","job":"Web Developer III","salary":"$3.62","location":"Malta"},
...
//1
const onlyWebDevs= emp.reduce((webDevs,eachObj) =>
{
if(eachObj.job.match("Web Developer")){
webDevs.push(eachObj);
}
return webDevs
},[])
console.log(onlyWebDevs)

//2
let onlySalary=emp.reduce((sal,eachObj) =>
{ 
sal.push(parseFloat((eachObj.salary).replace(/\$/g,"")))
return sal;
},[])
...
let newSalary=emp.reduce((sal,eachObj) =>
{ 
sal.push(parseFloat((eachObj.salary).replace(/\$/g,""))*1000)
return sal;
},[])
console.log(newSalary)
 ...
 let sum=emp.reduce((sal,eachObj) =>
 { 
 sal=(parseFloat((eachObj.salary).replace(/\$/g,"")))+sal
 return sal;
 },0)
 console.log(sum)
 ...
 let sumBasedOnCountry=emp.reduce((sal,eachObj) =>
 { 
let loc=eachObj.location;


if(!sal[loc]){
    sal[loc] = 0;
    sal[loc]=(parseFloat((eachObj.salary).replace(/\$/g,"")))+sal[loc];
...
return sal;
 ...
 console.log(sumBasedOnCountry)

 //6
 let averageBasedOnCountry=emp.reduce((sal,eachObj) =>
 { 
let loc=eachObj.location;


if(!sal[loc]){
    sal[loc] = {total: 0,count:0};
    sal[loc]['total']=(parseFloat((eachObj.salary).replace(/\$/g,"")))+sal[loc]['total'];
...
console.log(averageBasedOnCountry)
